...
class AriaForConditionalGeneration(AriaPretrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            print(f"--> Prompt: input_ids.shape: {input_ids.shape}, inputs_embeds.shape: {inputs_embeds.shape}")
        ...
        if pixel_values is not None:
            ...
            print(f"--> VT: pixel_values.shape: {pixel_values.shape}, pixel_mask.shape: {pixel_mask.shape}")
            print(f"image_outputs.last_hidden_state.shape: {image_outputs.last_hidden_state.shape}, image_attn_mask.shape: {image_attn_mask.shape}")

            selected_image_feature = image_outputs.last_hidden_state
            image_features = self.multi_modal_projector(
                selected_image_feature, attn_mask=image_attn_mask
            )
            print(f"--> Proj: selected_image_feature.shape: {selected_image_feature.shape}, image_attn_mask.shape: {image_attn_mask.shape}, image_features.shape: {image_features.shape}")
        ...
        if image_features is not None:
            ...
            print(f"--> Merge: n_image_tokens: {n_image_tokens} (self.config.image_token_index: {self.config.image_token_index}), special_image_mask.shape: {special_image_mask.shape}, inputs_embeds.shape: {inputs_embeds.shape}")
        ...
        #print(f"outputs: {outputs}, logits.shape: {logits.shape}")
        ...
        if labels is not None:
            ...
            print(f"--> Loss: attention_mask.shape: {attention_mask.shape}, shift_logits.shape: {shift_logits.shape}, shift_labels.shape: {shift_labels.shape}")
            # Flatten the tokens

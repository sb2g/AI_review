...
class FFN(nn.Module):
    ...
    def __init__(self, embed_dim, ff_dim, output_dim):
        ...
        print(f"projector FFN: embed_dim: {embed_dim}, ff_dim: {ff_dim}")
...
class CrossAttention(nn.Module):
    ...
    def __init__(self, kv_dim, embed_dim, num_heads, drop_out_rate=0):
        ...
        print(f"projector CrossAttention: embed_dim: {embed_dim}, kv_dim: {kv_dim}")
    ...
    def forward(self, x, hidden_states, attn_mask=None, add_residual=False):
        ...
        print(f"projector CrossAttention: hidden_states.shape: {hidden_states.shape}, query.shape: {query.shape}")
              
        x = self.ln_kv(x)
        key = self.k_proj(x).permute(1, 0, 2)
        value = self.v_proj(x).permute(1, 0, 2)
        print(f"projector CrossAttention: x.shape: {x.shape}, key.shape: {key.shape}, value.shape: {value.shape}, attn_mask.shape: {attn_mask.shape}")

        attn_output, _ = self.multihead_attn(query, key, value, attn_mask=attn_mask)
        print(f"projector CrossAttention: attn_output.shape {attn_output.shape}")

        attn_output = attn_output.permute(1, 0, 2)        
        print(f"projector CrossAttention: after permute: attn_output.shape: {attn_output.shape}")

        if add_residual:
            attn_output = hidden_states + self.dropout(self.linear(attn_output))
        else:
            attn_output = self.dropout(self.linear(attn_output))
        print(f"projector CrossAttention: after linear: attn_output.shape: {attn_output.shape}")
...
class AriaProjector(nn.Module):
    ...
    def forward(self, x, attn_mask=None):
        ...
        print(f"projector: x.shape: {x.shape}, queries.shape: {queries.shape}, query_num: {query_num}")

        if attn_mask is not None:
            attn_mask = attn_mask.repeat_interleave(self.num_heads, 0)
            attn_mask = attn_mask.unsqueeze(1).expand(-1, queries.size(1), -1)
            print(f"projector: attn_mask.shape: {attn_mask.shape}")

        attention_out = self.cross_attn(x, queries, attn_mask=attn_mask)
        print(f"projector: attention_out.shape: {attention_out.shape}")

        out = self.ffn(self.ln_ffn(attention_out))
        print(f"projector: out.shape: {out.shape}")

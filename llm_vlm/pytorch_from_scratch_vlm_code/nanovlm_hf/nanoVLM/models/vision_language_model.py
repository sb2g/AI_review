...
class VisionLanguageModel(nn.Module):
    ...
    def generate(self, input_ids, image, attention_mask=None, max_new_tokens=5, top_k=50, top_p=0.9, temperature=0.5, greedy=False):
        ...
        image_embd = self.vision_encoder(image)
        print(f"VE image_embd.shape: {image_embd.shape}")
        image_embd = self.MP(image_embd)
        print(f"MP image_embd.shape: {image_embd.shape}")

        # Embed initial tokens
        token_embd = self.decoder.token_embedding(input_ids)
        print(f"input_ids.shape: {input_ids.shape}, token_embd.shape: {token_embd.shape}")
        
        # Concatenate image embeddings with token embeddings
        combined_embd = torch.cat((image_embd, token_embd), dim=1)
        print(f"combined_embd.shape: {combined_embd.shape}")
        ...
        if attention_mask is not None:
            # Create mask of 1s for image tokens (all image tokens should be attended to)
            print(f"pre img attention_mask.shape: {attention_mask.shape}")
            image_attention_mask = torch.ones((batch_size, img_seq_len), device=attention_mask.device, dtype=attention_mask.dtype)
            attention_mask = torch.cat((image_attention_mask, attention_mask), dim=1)
            print(f"post img attention_mask.shape: {attention_mask.shape}")
        ...
        print(f"generated_tokens.shape: {generated_tokens.shape}, max_new_tokens: {max_new_tokens}")

        #Note: Here you could implement improvements like e.g. KV caching
        for i in range(max_new_tokens):
            model_out = self.decoder(outputs, attention_mask)
            print(f"outputs.shape: {outputs.shape}, model_out.shape: {model_out.shape}")
            if attention_mask is not None: print(f"attention_mask.shape: {attention_mask.shape}")
            ...
            if attention_mask is not None: print(f"attention_mask.shape: {attention_mask.shape}")
            print(f"next_embd.shape: {next_embd.shape}, outputs.shape: {outputs.shape}")
